...
async def rewrite_node(state: AgentState) -> AgentState:
    ...
    NOTA DE ASINCRONÍA (Fase 4 / Chainlit): este nodo es `async` y usa
    `.ainvoke()` (en vez de `.invoke()`) para que LangGraph pueda ejecutarlo
    dentro de `graph.astream_events(...)` sin bloquear el bucle de eventos
    de Chainlit. Si se usara `.invoke()` síncrono, la llamada de red a
    Ollama congelaría el event loop y la interfaz dejaría de responder
    mientras se genera esta reescritura.
    ...
    query_reescrita = (await chain.ainvoke(messages)).strip()
...
async def retrieve_and_rerank_node(state: AgentState, retriever: LegalRetriever) -> AgentState:
    """
    ...
    NOTA DE ASINCRONÍA (Fase 4 / Chainlit): `retriever.cercar()` es 100%
    síncrono y además es CPU-bound (BM25, encoding de embeddings, y sobre
    todo el forward pass del Cross-Encoder). Ejecutarlo directamente dentro
    de este nodo `async` bloquearía igualmente el bucle de eventos de
    Chainlit durante ese cómputo. Por eso lo delegamos a un hilo aparte con
    `asyncio.to_thread`, que libera el event loop mientras el retriever
    trabaja y permite que la UI (p. ej. el spinner del cl.Step) se siga
    ...
    """
    fragmentos_ganadores: List[ResultatFinal] = await asyncio.to_thread(
        retriever.cercar, state["query_reescrita"]
    )
...
async def evaluate_relevance_node(state: AgentState) -> AgentState:
    """
    ...
    NOTA DE ASINCRONÍA: `async` + `.ainvoke()` por el mismo motivo que en
    `rewrite_node` — es una llamada de red a Ollama y no debe bloquear el
    event loop mientras Chainlit atiende la interfaz.
    ...
    evaluation: RelevanceEvaluation = await structured_llm.ainvoke(messages)
...
async def generate_with_thinking_node(state: AgentState) -> AgentState:
    """
    ...
    NOTA DE ASINCRONÍA (clave para Chainlit): este nodo usa `.ainvoke()`,
    que basta para no bloquear el event loop. El STREAMING token a token
    que pide la Fase 4 no se implementa aquí dentro del nodo, sino que lo
    captura `app.py` a nivel de LangGraph escuchando los eventos de tipo
    `on_chat_model_stream` que emite `graph.astream_events(...)` cuando
    detecta que este nodo (`generate_with_thinking`) invoca un ChatModel.
    Gracias a esto, `app.py` puede separar en tiempo real el contenido
    dentro de `<think>...</think>` del resto de la respuesta sin que este
    nodo tenga que saber nada sobre Chainlit ni sobre UI.
    ...
    respuesta_final = (await chain.ainvoke(messages)).strip()
...
def build_graph(retriever: "LegalRetriever"):
    ...
    async def _retrieve_and_rerank_with_retriever(state: AgentState) -> AgentState:
        return await retrieve_and_rerank_node(state, retriever)
...
async def _run_demo() -> None:
    """Ejecuta una consulta de ejemplo de extremo a extremo (uso por CLI)."""
    ...
    # 4. Ejecutamos el flujo completo del grafo. Al ser todos los nodos
    #    `async`, invocamos con `.ainvoke()` en vez de `.invoke()`.
    resultado_final: AgentState = await agent_graph.ainvoke(initial_state)
    ...
    for i, doc in enumerate(resultado_final["documentos"], start=1):
        print(f"  [{i}] {doc['titol_chunk']} (score={doc['score_cross_encoder']:.4f})")
